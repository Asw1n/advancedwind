...
There are in fact two other frames of reference that are relevant to the plugin. One is the mast as the mast moves in respect to the vessel on a rocking boat. The other is the wind sensor as the wind sensor can be misaligned in respect to the vessel. To the plugin the wind sensor does not measure apparent wind but sensor wind. 
...
The apparent wind that the wind sensor measures does not contain all the corrections the plugin made. To apply the corrections to apparent wind too, it is back calculated from the true wind: vessel_wind = water_wind + vessel speed. The resulting apparent wind has all the corrections that are done when calculating true wind, except for the leeway correction.
The apparent wind that the wind sensor provides is filtered out by the plugin. Instead the plugin will provide the corrected apparent wind in the paths environment.wind.speedApparent and environment.wind.angleApparent.
